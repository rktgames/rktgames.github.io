class Settings extends Phaser.Scene {
    ...
    create() {
        ...
        var resume = this.add.image(550, 300, 'settingsButton').setDisplaySize(200, 60).setInteractive();
        var resumeText = this.add.text(520, 290, 'Resume', { font: '20px comic sans' });
        resume.on("pointerup", function () {
            this.scene.start(this.gameName);
        }, this)

        var reset = this.add.image(550, 350, 'settingsButton').setDisplaySize(200, 60).setInteractive();
        var resetText = this.add.text(530, 340, 'Reset', { font: '20px comic sans' });
