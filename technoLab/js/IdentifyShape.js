class IdentifyShape extends Phaser.Scene {
    ...
    generateShapes() {
        this.question_Text = this.add.text(100, 65, '', { font: '30px Courier', fill: '#fff' });
        this.selectItem = '';
        this.createdElements = [];
    ...
    createSquare(x, y) {
        ...
        var square = this.add.rectangle(x, y, width, width, fillColor).setInteractive().on('pointerup', () => this.clickedShape('square'));
        square.name = 'square';
        return square;
    }
    ...
    createRectangle(x, y) {
        ...
        var fillColor = getRandomColor();
        var rectangle = this.add.rectangle(x, y, width, height, fillColor).setInteractive().on('pointerup', () => this.clickedShape('rectangle'));
    ...
    createCircle(x, y) {
        ...
        var fillColor = getRandomColor();
        var circle = this.add.circle(x, y, radius, fillColor).setInteractive().on('pointerup', () => this.clickedShape('circle'));
        circle.name = 'circle';
        return circle;
    }
    ...
    createArc(x, y) {
        ...
        var fillColor = getRandomColor();
        var arc = this.add.arc(x, y, radius, startAngle, endAngle, anticlockwise, fillColor).setInteractive().on('pointerup', () => this.clickedShape('arc'));
        arc.name = 'arc';
        return arc;
    }
    ...
    createEllipse(x, y) {
        ...
        var ellipse = this.add.ellipse(x, y, width, height, fillColor).setInteractive().on('pointerup', () => this.clickedShape('ellipse'));
    ...
    createTriangle(x, y) {
        ...
        triangle.setInteractive().on('pointerup', () => this.clickedShape('triangle'));
    ...
    createPentagon(x, y) {
        ...
        var pentagon = this.add.polygon(x, y, points, fillColor).setInteractive().on('pointerup', () => this.clickedShape('pentagon'));
    ...
    createHexagon(x, y) {
        ...
        var hexagon = this.add.polygon(x, y, points, fillColor).setInteractive().on('pointerup', () => this.clickedShape('hexagon'));
    ...
    resetStage() {
        this.question_Text.setText('').setStyle({ fill: '#fff' });
        for (var i = 0; i < this.createdElements.length; i++) {
